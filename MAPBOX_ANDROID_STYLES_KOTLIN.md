...
object MapboxStyles {
    ...
    // Estilos Clásicos
    const val STREETS           = "mapbox://styles/mapbox/streets-v12" // ⭐ AdminComplexes (formulario de registro)
    const val OUTDOORS          = "mapbox://styles/mapbox/outdoors-v12"
    const val LIGHT             = "mapbox://styles/mapbox/light-v11"
    const val DARK              = "mapbox://styles/mapbox/dark-v11"   // ⭐ MapDialog / MapCard (jugador)
...
La web usa estilos distintos según el contexto:

| Pantalla web | Estilo |  
|---|---|
| `MapDialog` / `MapCard` mini (Home — jugador) | `DARK` (`dark-v11`) |
| `Complexes` — mapa embebido de exploración | `STREETS` (`streets-v12`) |
| `AdminComplexes` — formulario de registro | `STREETS` (`streets-v12`) |
...
class MapFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Mapa principal Home (jugador) → estilo oscuro
        mapView.getMapboxMap().loadStyleUri(MapboxStyles.DARK) { style ->
            setupMarkers(style)
        }

        // Exploración de complejos (Complexes) y formulario admin → calles legibles
        // mapView.getMapboxMap().loadStyleUri(MapboxStyles.STREETS) { style -> ... }
    }
}
```
...
| Cargar estilo (Home/jugador) | `MAPBOX_STYLES.dark` → `MapCard` / `MapDialog` | `loadStyleUri(MapboxStyles.DARK)` |
| Cargar estilo (exploración/admin) | `MAPBOX_STYLES.streets` → `Complexes`, `AdminComplexes` | `loadStyleUri(MapboxStyles.STREETS)` |
